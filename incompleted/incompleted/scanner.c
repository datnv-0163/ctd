...
// Truong hop doc vao la 1 so thi se ep kieu token->string thanh so va gan vao token->value
...
Token* getToken(void) {
  ...
  switch (charCodes[currentChar]) {
  case CHAR_SPACE: skipBlank(); return getToken();        // Doc dau cach thi bo di, tiep tuc doc token moi
